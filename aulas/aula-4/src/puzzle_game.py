...
class PuzzleGame:
    ...
    def get_tile(self, line, column):
        print("1 Inicializando valores de entrada")
        print(f"2 Verificando se a posição ({line}, {column}) está dentro do tabuleiro", end=" ")
        if line > 0 and line <= self.board.number_of_lines and \
                column > 0 and column <= self.board.number_of_columns:
            print("(T)")
            print(f"4 Verificando se a posição ({line}, {column}) é a posição vazia", end=" ")
            if line == self.line_of_empty_position and \
                column == self.column_of_empty_position:
                print("(T)")
                print("5 Returnando string vazia")
                return (" ")
            else:
                print("(F)")
                print(f"6 Retornando tile da posição ({line}, {column})")
                return self.board.get_tile(line, column)
        else:
            print("(F)")
            print("3 Disparando exceção")
